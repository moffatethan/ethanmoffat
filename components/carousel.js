...
const ProjectCarouselSlide = ({ logoUrl, projectImageUrl, description, usedTech, isConcept }) => (
  <div className='bg-white dark:bg-gray-200 shadow-lg rounded-3xl p-8'>
    <div className='flex px-5'>
      <div>
        ...
        <div className="mt-12 mr-8 text-xl text-gray-400 font-medium leading-10">
          <p className="mb-4">Tech Used: {usedTech.join(", ")}</p>
          <p>Concept: {isConcept ? 'Yes' : 'No' }</p>
        </div>
      </div>
      <img className='hidden lg:block flex-1 h-96 shadow-md rounded-3xl' src={projectImageUrl} />
    </div>
    <button className='w-full md:w-auto mt-12 bg-blue-500 border shadow-md border-blue-500 text-blue-100 text-md px-12 py-4 rounded-lg transition hover:bg-blue-600 transform hover:-translate-y-1'>View Project</button>
  </div>
)
...
const CarouselSlide = ({ imgUrl, title, description, small = true }) => (
  <div className="bg-white dark:bg-gray-200 shadow-lg rounded-3xl px-8 py-6">
    <div className='block md:flex md:h-48'>
              ...
              <div key={nanoid()} className='p-2 md:p-8 w-full'>
                {
                  ...
                  : <ProjectCarouselSlide logoUrl={slide.projectLogo?.url} title={slide.title} description={slide.description} projectImageUrl={slide.projectImage?.url} small={false} isConcept={slide.isConcept} usedTech={slide.metadata.techUsed} />
                }
              </div>
            )
          )
